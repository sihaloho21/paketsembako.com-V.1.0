...
import { useGetUser, useGetPointsHistory } from "@/hooks/use-gas-api";
import { useUserId } from "@/hooks/use-user-id";
import { LevelCardSkeleton, TextSkeleton } from "@/components/skeletons";
...
export default function Level() {
  ...
  const { data: pointsHistory } = useGetPointsHistory(userId);

  // Determine current level index based on user XP
  const getCurrentLevelIndex = () => {
    ...
    for (let i = levelOrder.length - 1; i >= 0; i--) {
      const threshold = LEVEL_THRESHOLDS[levelOrder[i] as keyof typeof LEVEL_THRESHOLDS];
      if (userXP >= threshold) {
        return i;
      }
    }
  ...
  const userXP = user?.xp || 0;
  const currentThreshold = LEVEL_THRESHOLDS[currentLevel.name as keyof typeof LEVEL_THRESHOLDS] || 0;
  const nextLevelThreshold = currentLevelIndex < levels.length - 1 
    ? LEVEL_THRESHOLDS[levels[currentLevelIndex + 1].name as keyof typeof LEVEL_THRESHOLDS]
    : currentLevel.xpNeeded;
  const xpInCurrentLevel = userXP - currentThreshold;
  const xpNeededForNextLevel = nextLevelThreshold - userXP;
  const progressPercentage = Math.min(
    ((userXP - currentThreshold) / (nextLevelThreshold - currentThreshold)) * 100,
    100
  );

  const isLocked = activeLevel > currentLevelIndex;
    ...
    <div className="min-h-full bg-white">
        ...
        <div className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide px-4 gap-3 pt-4 pb-2">
          {isLoadingUser ? (
            ...
            levels.map((lv, i) => {
              ...
              const lvXP = userXP;
              const lvCurrentThreshold = LEVEL_THRESHOLDS[lv.name as keyof typeof LEVEL_THRESHOLDS] || 0;
              const lvNextThreshold = i < levels.length - 1 
                ? LEVEL_THRESHOLDS[levels[i + 1].name as keyof typeof LEVEL_THRESHOLDS]
                : lv.xpNeeded;
              const lvProgress = isUnlocked
                ? Math.min(((lvXP - lvCurrentThreshold) / (lvNextThreshold - lvCurrentThreshold)) * 100, 100)
                : 0;
              ...
              return (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    isUnlocked ? "from-green-50 to-green-100" : "from-gray-100 to-gray-200"
                  )}>
                    {/* Background circle */}
                    <div className={cn(
                      "absolute bottom-0 left-1/2 -translate-x-1/2 w-56 h-40 rounded-t-full",
                      isUnlocked ? "bg-green-200/60" : "bg-gray-300/50"
                    )} />
                    ...
                    {isUnlocked && (
                      <div className="absolute top-3 right-3 bg-white rounded-full px-2.5 py-1 flex items-center gap-1 shadow">
                        ...
                        <span className="text-[11px] font-black text-foreground">XP {lvXP}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className={cn("px-3 py-2 flex items-center gap-2", lv.labelBg)}>
                    <Sprout size={16} className={isUnlocked ? "text-green-600" : "text-gray-500"} />
                  ...
                  {isUnlocked ? (
                    <div className="bg-white px-3 py-3">
                      ...
                      <p className="text-xs text-center text-muted-foreground mb-2 font-medium">
                        {i < levels.length - 1
                          ? `Kumpulkan +${Math.max(0, xpNeededForNextLevel)} XP lagi untuk naik level!`
                      ...
                      <div className="flex justify-between items-center mt-1">
                        <span className="text-[10px] text-muted-foreground">
                          {xpInCurrentLevel} / {nextLevelThreshold - currentThreshold} XP
                        </span>
          ...
          {level.rewards.map((r, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5 w-[72px]">
              <div className={cn(
                ...
                isLocked ? "bg-gray-200" : "bg-gradient-to-br from-green-500 to-green-700"
              )}>
                <div className={isLocked ? "text-gray-400 grayscale" : "text-white"}>
                  {r.icon}
                </div>
                {r.badge && !isLocked && (
      ...
      <div className="mt-4 mx-4 mb-6 bg-white border border-border/60 rounded-xl overflow-hidden shadow-sm">
        <Link href="/xp-history">
          <div className="w-full flex items-center justify-between px-4 py-4 hover:bg-slate-50 transition-colors cursor-pointer">
            <div>
              <p className="text-sm font-bold text-foreground text-left">Riwayat XP</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {isLoadingUser ? "Loading..." : `${pointsHistory?.length || 0} transaksi tercatat`}
              </p>
            </div>
